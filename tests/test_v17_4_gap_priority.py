...
class V174GapPriorityTests(unittest.TestCase):
    ...
    def test_scanner_prioritises_sparsest_cells_without_changing_admission(self):
        ...
        self.assertIn("Empty cells are", scanner)
        self.assertEqual(config.get("frontier_gap_queries_per_scan"), 6)
        self.assertEqual(len(config.get("frontier_gap_search_queries", {})), 16)
        self.assertEqual(config.get("frontier_gap_scholarly_queries_per_scan"), 10)
        self.assertEqual(len(config.get("frontier_gap_scholarly_queries", {})), 16)
        self.assertIn('knowledge-D', config.get('frontier_gap_priority_cells', []))
        ...
        self.assertIn("if(materiality<2.4 || qCount===0", frontier)
