...
class ExpansionOnlyStateTests(unittest.TestCase):
    def test_bundled_radar_is_supplied_current_state(self):
        ...
        self.assertEqual(len(data.get('strand_a', [])), 48)
        self.assertEqual(len(data.get('strand_b', [])), 10)
        self.assertEqual(len(data.get('strand_c', [])), 5)
        self.assertEqual(data.get('last_updated'), '2026-08-23T14:49Z')
        self.assertEqual(data['scan_state']['crossref_broad_cursor'], 49)
        self.assertEqual(data['scan_state']['crossref_priority_cursor'], 225)
