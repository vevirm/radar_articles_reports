...
class V12SourceExpansionTests(unittest.TestCase):
    def test_first_v12_scan_uses_four_calendar_months(self):
        today = dt.date(2026, 8, 20)
        from_date, bootstrap = s.scan_from_date({}, today)
        self.assertTrue(bootstrap)
        self.assertEqual(from_date, dt.date(2026, 4, 20))

    def test_upgrade_without_v12_marker_forces_backfill(self):
        today = dt.date(2026, 8, 20)
    ...
    def test_second_v12_scan_uses_overlap_not_four_months(self):
        today = dt.date(2026, 8, 20)
        previous = {
