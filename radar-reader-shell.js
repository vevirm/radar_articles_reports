...
(()=>{
  ...
  const side=document.createElement('aside');side.className='radar-shell-sidebar';side.innerHTML=`<a class="radar-shell-brand" href="../">R&amp;I × Geopolitics Radar</a><nav>${links}</nav><div class="radar-shell-foot">Several ways to explore the same material.</div>`;document.body.insertBefore(side,document.body.firstChild);
  ...
  (globalThis.RadarData?.load?RadarData.load('../radar.json','../radar_seed.json'):Promise.reject(new Error('RadarData unavailable'))).then(d=>{const rows=[...(d.strand_a||[]),...(d.strand_b||[]),...(d.strand_c||[])],fresh=new Set(rows.filter(x=>x&&x.new_this_scan).map(x=>clean(x.link||x.url)||clean(x.title||x.headline)).filter(Boolean)).size,sources=new Set(rows.map(x=>clean(x.source)).filter(Boolean)).size;document.getElementById('radarShellFacts').innerHTML=fact(fmt(rows.length),'active records')+fact(fmt(fresh),'new last scan',fresh>0)+fact(fmt(sources),'sources')+fact(fmtDate(d.run_completed_at||d.last_updated),'last scan')}).catch(()=>{document.getElementById('radarShellFacts').innerHTML='<span>Active counts unavailable.</span>'});
})();
