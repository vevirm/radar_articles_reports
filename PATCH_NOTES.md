...
- Fixes run-trigger telemetry under an old retained workflow by falling back to GitHub's native `GITHUB_EVENT_NAME`. Manual runs now record `manual`; scheduled runs record `scheduled`; push/rescue remain distinct.
- Tightens Strand-A centrality for the four false positives exposed by the 20:13 manual run: award/prize landing pages, Horizon-Europe-as-project-provenance papers, R&D-spending-as-a-covariate papers, and local/service innovation studies do not become A merely through generic R&I wording.
- Keeps the substantive European Biotech Act staff working document from that run. The bundled public outcome is therefore corrected from 5 new A to **1 retained new A**; the four confirmed false positives are removed and protected from Git-history resurrection.
- Strengthens Nature as a real direct discovery source. In addition to Nature RSS and article pages, the scanner now extracts publisher-hosted title/teaser/date metadata directly from Nature's news hub and `latest-news` surface. This covers same-day `d41586-*` news that can appear on the hub before the Nature News RSS catches up. A full RSS link budget no longer suppresses the newer hub-card check.
- Expands the external talent-signal vocabulary for scientist/researcher return schemes, re-entry fellowships and brain-circulation moves. The Mohana Basu Nature pattern now reaches the talent theme and can anchor to substantive European talent evidence instead of being misrouted to a broad critical-technology anchor.
- Makes C anchoring claim-aware: when a story contains several background technologies, the A anchor is preferentially selected for the mechanism actually carried by the publishable `What happened` sentence. The public signal kind is also classified from that event claim, so a scientist-return programme stays `research / talent` even when the teaser also mentions AI, quantum, biotech or materials funding.
- Adds non-circular Matrix telemetry. The exact browser classifier still decides placements; automatically placed A/B/frontier records receive a display-only `matrix_auto_cell`, while evidence/adjudication fields (`matrix_dimension`, `matrix_quadrant`) are untouched. The JSON also reports new-A/new-B Matrix placements and published undercoverage.
- Bumps only the substantive A quality profile and the C discovery profile. The expensive OpenAlex/Crossref/institution cursors and recall profile are **not reset**. The changed C discovery profile intentionally triggers one bounded recovery pass so missed current Nature/talent signals can be found automatically.
- Four-hour current workflow, cumulative A/B/frontier retention, 60-day-from-`first_seen` C retention and the repository credential/write boundary are unchanged.
