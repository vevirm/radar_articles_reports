# v17.20.47 — fresh start: keep the 200 most important items, restart discovery, make snowballing work

- **Root cause of the dead citation snowball.** Every recent scan reported `seeds_planned: 20, seeds_resolved: 0`. The seed pool sorted by source tier first, and 288 of the 600 saved Strand-A rows were "Tier 1" institutional news/notice pages (ERA Portal news items, JRC repository pages, generic Commission topic pages) that have no DOI and do not exist in OpenAlex. They filled all twenty seed slots on every scan, so the 263 DOI-backed publications never got a turn and no shared-reference anchor could ever form.
- **Snowball repair.** Seeds must now be resolvable: only DOI-backed accepted Strand-A publications (or explicitly pinned titles) are seeds (`citation_snowball_require_doi`). All DOI seeds are resolved in one batched OpenAlex request per 50 DOIs instead of twenty separate lookups. A persisted cursor in `scan_state.citation_snowball` walks the whole seed pool across successive scans so the lane genuinely snowballs through the corpus, seeds that failed to resolve are skipped for `citation_snowball_unresolvable_retry_days`, resolved OpenAlex ids and selected anchors are remembered. Admission is unchanged: every discovered work still passes the ordinary EU + R&I + strategic-context gates.
- **One-time fresh-start corpus reset (`corpus_reset_profile_version`).** The scanner keeps the `corpus_reset_keep_items` (200) most important accepted A/B items and discards the rest. Importance is a ranking over fields already saved in radar.json: curator/manual-verified material, Matrix-placed evidence, DOI-backed peer-reviewed work, explicit geopolitical/strategic evidence and recency score highest; short administrative notices (appointments, calls, registrations, winners) and aggregator re-posts score lowest. A small Strand-B floor (`corpus_reset_min_strand_b_items`) keeps the foresight-method strand alive. The full report, including every pruned title, is persisted under `corpus_reset` in radar.json.
- **Discovery restarts from the beginning.** The reset replaces `scan_state` with a fresh state: all OpenAlex/Crossref/institution/frontier/priority-people cursors return to 0, per-family backfill flags reopen so the scanner performs its full four-month bootstrap pass again, `cycle_failed`, result-depth pages and the institutional seen-URL cache are cleared. Scheduling timestamps, resolved OpenAlex author ids and one-time source-recovery completion markers are kept because repeating them adds nothing.
- **Pruned items cannot creep back.** Their DOI/title identities are remembered (`corpus_reset_block_pruned_readmission`) and refused by `merge_corpus()` and the low-yield "genuinely new" counter, so a restarted rotation that rediscovers the same notice pages does not refill the corpus with them.
- **Git-history recovery respects the reset epoch.** `load_previous()` used to merge the largest radar.json from the last 60 commits back into the current one ("never lose a row") and to prefer the newer `scan_state`; that would have silently resurrected the 600-item corpus and the old cursors on the next whole-repository upload. Snapshots from a different `corpus_reset.version` are now ignored in both directions: pre-reset history cannot re-inflate a reset bundle, and a stale pre-reset bundle cannot overwrite a repository that already reset.
- **Workflow safety gate.** `radar-scan.yml` accepts `corpus_reset_this_run` as a deliberate cleanup, and the scanner also raises the legacy `quality_migration_this_run` flag on the reset run so a stale hidden workflow still commits the result. The shipped radar.json is already reset (`scripts/reset_corpus.py`), so the first GitHub scan starts from the retained 200 items and the fresh state; if only code is uploaded, the scanner applies the identical reset itself on its first run.
- **No page changes.** HTML/JS readers are untouched; they simply render a smaller, higher-quality corpus.
- Regression coverage: `tests/test_v172047_fresh_start_and_snowball.py` (importance ranking, reset/state restart, readmission block, Git-epoch guard in both directions, DOI-first rotating seeds, batched resolution feeding shared-reference anchors, 429 reporting, packaging markers).
