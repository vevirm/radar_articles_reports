...
## Evidence briefing layer (post-admission)

`findings.html` / `findings.json` do **not** change any admission rule above. They are a post-admission presentation layer. For each accepted A/B publication, the scanner selects one source-level sentence from the same text/evidence already used by the gate; Strand A prioritises the R&I–geopolitics bridge and Strand B prioritises foresight-plus-method evidence. Older corpus entries are backfilled from their stored summaries. Recurring-theme cards report corpus recurrence plus one representative finding rather than asserting an automatically generated consensus. Strand C briefing bullets remain tied to the same explicit A/B anchors required above.
